<div class="tab-pane fade show active" id="agri" role="tabpanel" aria-labelledby="agri-tab">
  <?php
  // Fetch unique files with activities limited to folder_id = 1
  $fileQuery = "
  ...
  if (mysqli_num_rows($fileResult) > 0) {
    while ($file = mysqli_fetch_assoc($fileResult)) {
      ...
      // Fetch activities per file filtered by folder_id = 1
      $activityQuery = "
        SELECT 
            fa.id AS audit_id,
            fa.file_id,
        ...
        WHERE fa.file_id = '$fileId' AND fa.folder_id = 1
      ...
      if (mysqli_num_rows($activityResult) > 0) {
        ...
        echo '<p class="text-muted text-center">No activities found for this file in Folder 1.</p>';
      }

      echo '</div></div></div>';
    }
  } else {
    echo '<div class="text-center p-4"><h6 class="text-muted">No files with activity logs found in Folder 1.</h6></div>';
  }
  ?>
</div>
